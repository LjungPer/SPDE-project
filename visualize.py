...
def coef_from_image(file, beta = None, alpha = None, rel_rb = None, rel_bg = None):

    temp = image.imread(file)

    if rel_rb is None:
        rel_rb = 10 # Weight scale between red and blue
    if rel_bg is None:
        rel_bg = 10 # Weight scale between blue and green
    if beta is None:
        beta = 5
    if alpha is None:
        alpha = 1

    # Ensure the maximum of the coefficient is beta (after adding alpha in the end)
    r = (beta - alpha) / (1 + rel_rb + rel_rb * rel_bg)
    b = rel_rb * r
    g = rel_bg * b

    # Choose how to scale rgb colors of each pixel
    scale = np.array([r, g, b, 0])

    # Create coefficient
    coef = np.dot(temp, scale) + alpha
...
def drawCoefficientwt(N, a, greys=False):
    ...
    plt.imshow(aCube, origin='upper', interpolation='none', cmap=cmap)
    plt.tick_params(axis='both', which='both', bottom='off', top='off', labelbottom='off', right='off', left='off', labelleft='off')
